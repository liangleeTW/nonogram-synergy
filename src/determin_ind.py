...
def solve_ind_turn_based_logged(
    row_clues: List[List[int]],
    col_clues: List[List[int]],
    strategy: str = "first",
    max_turns: int = 1000,
    ...
    while turn < max_turns and not is_solved(grid):
        ...
        move = ind_agent_move(grid, row_clues, col_clues, strategy=strategy)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--row-strategy",
        choices=["first", "random", "most_constrained"],
        help="Optional alias for --strategy to keep the interface close to src/determin_dyad.py.",
    )
    parser.add_argument(
        "--col-strategy",
        choices=["first", "random", "most_constrained"],
